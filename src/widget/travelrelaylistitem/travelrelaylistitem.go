...
type Widget struct {
	...
	data api.TravelRelayResponse

	featuresList string
}

func Constructor(data api.TravelRelayResponse) widgetlist.ListItem[api.TravelRelayResponse] {
	w := new(Widget)
...
func (w *Widget) UpdateData(data api.TravelRelayResponse) {
...
func (w *Widget) GetData() api.TravelRelayResponse {
	return w.data
}
...
func (w *Widget) Render() string {
	...
	right := t.Style(theme.NormalTextStyleID).
		...
		Render(fmt.Sprintf("%s %c %d%c",
			helper.HoursDecFormat(w.data.Duration),
			art.CharBullet,
			w.data.Price,
			art.CharGrynars))
